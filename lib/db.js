...
;(async function initDb () {
  ...
  try {
    ...
    const indexes = {
      ...
      statistics: ['jobExecutionId', 'seedId', 'entityId']
    }
    ...
    if (!userExists) {
      ...
      // grant user write permissions on extracted_text table
      log.info('Granting user', dbUser, 'read/write permissions for: veidemann.extracted_text table')
      await r.db('veidemann').table('extracted_text').grant(dbUser, {read: true, write: true})

      log.info('Granting user', dbUser, 'read permissions for: veidemann.config_crawl_entities')
      await r.db('veidemann').table('config_crawl_entities').grant(dbUser, {read: true})

      log.info('Granting user', dbUser, 'read permissions for: veidemann.config_seeds')
      await r.db('veidemann').table('config_seeds').grant(dbUser, {read: true})
    }
